...
SolutionCalculator::SolutionCalculator(const LUDecomposition &_lu, std::ifstream *_bVecFile,
                                       const int _myid, const int _numProcs, const unsigned _n) : L(_lu.getL()), U(_lu.getU()), myid(_myid),
...
void SolutionCalculator::calculateYVector()
{
    ...
    upcxx::reduce_all(yPartial, y, n, upcxx::experimental::op_add).wait();
...
void SolutionCalculator::calculateXVector()
{
    ...
    upcxx::reduce_all(xPartial, x, n, upcxx::experimental::op_add).wait();
